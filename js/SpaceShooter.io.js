function SpaceShooter(io){
            ...
            bgImgs[i].onload = function(){
                ...
                for (var j=0; j<bgDensity; j++)
                    if (iio.getRandomNum() < .4){
                        obj = new iio.ioRect(iio.getRandomInt(10, io.canvas.width-10)
                                            ,iio.getRandomInt(0, io.canvas.height));
                        obj.createWithImage(bgImgs[this[0]])
                           .enableKinematics()
                           .setVel(0,vel)
                           .setBound('bottom',io.canvas.height+140
                                     ,moveToTop);
                        io.addToGroup(tag,obj,zIndex)
                    }
            }.bind([i])
    })();
    ...
    (function(){
        ...
        updatePlayer = function(){
            ...
            //update lasor cannons
            if (input[SPACE] && lasorTimer < 0){
                fireLasor(player.left()+10, player.pos.y);
                fireLasor(player.right()-8, player.pos.y);
                lasorTimer = lasorCooldown;
            } 
            if (input[SPACE]) lasorTimer--;
            else lasorTimer-=3;
        }
        ...
        fireLasor = function(x,y){
            io.addToGroup('lasors', new ioRect(x,y),-1)
