...
(function (io) {
   ...
   io.getElementPosition = function(element) {
...
//Attaches iio Physics Framework
(function () {
   var ioVec = iio.ioVec
      ,ioObj = iio.ioObj;
   ioObj.prototype.update = function(dt){
   ...
   ioObj.prototype.setVel = function(v,y){
      if (v instanceof ioVec)
         this.vel = v;
   ...
   ioObj.prototype.setTorque = function(t){
      this.torque = t;
      if (typeof this.rotation == 'undefined')
         this.rotation=0;
      return this;
   }
   ioObj.prototype.setBounds = function(top, right, bottom, left, callback){
...
(function() {
   var ioObj = iio.ioObj;
   ioObj.prototype.draw = function(ctx,pos,r){
      ctx.save();
      ctx.webkitImageSmoothingEnabled=true;
      if (this.partialPixels) ctx.translate(pos.x, pos.y);
   ...
   function draw(ctx, pos, r, noRestore){
      ioObj.prototype.draw.call(this,ctx,pos,r);
   ...
   ioRect.prototype.draw = draw;
   if (b2PolygonShape != 'undefined'){
      b2PolygonShape.prototype.draw = draw;
      b2CircleShape.prototype.draw = draw;
   }
   ...
   ioRect.prototype.addImage = addImage;
   if (b2PolygonShape != 'undefined'){
      b2PolygonShape.prototype.addImage = addImage;
      b2CircleShape.prototype.addImage = addImage;
   }

   function addAnim(srcs){
   ...
   ioRect.prototype.addAnim = addAnim;
   ...
   ioRect.prototype.createWithImage = createWithImage;

   function createWithAnim(srcs, i, onLoadCallback){
   ...
   ioRect.prototype.createWithAnim = createWithAnim;   
})();


/* :::::::::::::::::::::::::::::::::::::::
 * iio Application Manager
 */
   ...
   ioGroup.prototype.draw = function(ctx){
      for (var i=0; i<this.objs.length; i++){
         if (typeof this.objs[i].pos!='undefined')
            this.objs[i].draw(ctx, this.objs[i].pos, this.objs[i].rotation);
         else if (typeof this.objs[i].m_I!='undefined')
            for (f=this.objs[i].GetFixtureList();f;f=f.m_next){
               s=f.GetShape(); 
               if (typeof s.draw!='undefined')
                  s.draw(ctx,new ioVec(this.objs[i].m_xf.position.x*30,this.objs[i].m_xf.position.y*30),this.objs[i].GetAngle());
           }
      }
   }
})();
