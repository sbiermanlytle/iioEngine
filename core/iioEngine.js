...
//UTIL LIBRARIES
   ...
   iio.v = {
      add : function(v1, v2){for(var p in v2) if(v1[p]) v1[p]+=v2[p];return v1},
      sub : function(v1, v2){for(var p in v2) if(v1[p]) v1[p]-=v2[p];return v1},
      mult: function(v1, v2){for(var p in v2) if(v1[p]) v1[p]*=v2[p];return v1},
      div : function(v1, v2){for(var p in v2) if(v1[p]) v1[p]/=v2[p];return v1},
      dist: function(v1, v2){return Math.sqrt(Math.pow(v2.x-v1.x,2)+Math.pow(v2.y-v1.y,2))}
   }
   ...
   iio.api = {
      run : function(s,nd){ return iio.run(s,o,nd) },
      ...
      if(o1 instanceof Array){
         if(o2 instanceof Array){
            ...
            o1.forEach(function(_o1) {
              o2.forEach(function(_o2) { if(iio.collision.check(_o1,_o2)) fn(_o1,_o2); });
            });
         } else {
            o1.forEach(function(_o1) { if(iio.collision.check(_o1,o2)) fn(_o1,o2); });
         }
      } else {
         if(o2 instanceof Array){
            o2.forEach(function(_o2) { if(iio.collision.check(o1,_o2)) fn(o1,_o2); });
...
//SHAPES
   ...
   iio.rect = {
      ...
      draw : function(ctx){
         ...
         if(this.bezier){
            ...
            iio.draw.rect(ctx,this.width,this.height,{c:this.color,o:this.outline},{img:this.img,anims:this.anims,animKey:this.animKey,animFrame:this.animFrame,mov:this.mov,round:this.round});
         }
         if(this.xColor){
   ...
   iio.grid = {
      ...
      draw : function(ctx){
         iio.draw.prep_shape(ctx,this);
         ctx.translate(-this.width/2,-this.height/2);
         iio.draw.rect(ctx,this.width,this.height,{c:this.color,o:this.outline},{img:this.img,anims:this.anims,mov:this.mov,round:this.round});
         if(this.gridColor){
